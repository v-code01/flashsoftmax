...
results/bench.jsonl and re-asserts the four claims with its own arithmetic:
  ...
  1. Online error tracks two-pass at every length (ratio <= 2) with no upward trend in length - the
     flash rescale is numerically free.
  2. Standalone, online is NOT faster than two-pass (median latency ratio >= 1) - it recomputes exps.
  3. Naive overflow onset increases with sequence length at large scale (monotone non-decreasing, and
     reaches every seed by the longest length).
...
def main() -> int:
    ...
    # 1. online/two-pass error ratio bounded and non-trending across length.
    ratios_by_len = []
    for L in lengths:
        rs = [r for c in (10,) for r in cell(L, c)]
        tp = med([_f(r["twopass_err"]) for r in rs])
        on = med([_f(r["online_err"]) for r in rs])
        ratios_by_len.append(on / tp if tp > 0 else 1.0)
    worst = max(ratios_by_len)
    # no upward trend: the ratio at the longest length is not markedly above the short-length ratios.
    trend_ok = ratios_by_len[-1] <= 2.0 and worst <= 2.5
    print(f"  [1] online/two-pass err ratio: worst {worst:.2f}, at L={lengths[-1]} "
          f"{ratios_by_len[-1]:.2f} (free & no length drift={trend_ok})")
    ok = ok and trend_ok
    ...
    # 3. naive overflow onset non-decreasing with length at scale=30, reaching 100% by the end.
    ov = []
    for L in lengths:
        c = cell(L, 30)
        ov.append(sum(1 for r in c if r["naive_err"] is None) / len(c))
    non_decreasing_trend = ov[-1] == 1.0 and ov[-1] >= ov[0]
    print(f"  [3] naive overflow fraction @scale=30: L={lengths[0]} {ov[0]:.0%} -> "
          f"L={lengths[-1]} {ov[-1]:.0%} (grows with length={non_decreasing_trend})")
    ok = ok and non_decreasing_trend
    ...
    if ok:
        print("VERIFY OK: online (flash) softmax matches two-pass accuracy with no length drift, is "
              "not faster standalone (recomputes exps), and naive overflow grows with length - all "
