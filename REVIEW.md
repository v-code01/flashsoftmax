...
length trend; the rigorous instrument uses a long-double oracle and a pure-f32 online path and shows no
trend. Reporting the misleading pilot and why it was wrong is what keeps the final claim honest, rather
than quietly dropping it.

## "Claiming flash attention's softmax is 2.8x slower is misleading - flash attention is fast."
The claim is carefully scoped: the *standalone* online softmax is 2.8x slower because it recomputes
exponentials. The study explicitly says this is not a claim that flash attention is slow - its win is
...
## "A long-double reference is not exact - your errors are against an approximation."
Long double on this platform carries ~64 bits of mantissa versus f32's 24, so the reference error is
~2^-40 of the f32 error being measured - four billion times smaller. For the purpose of comparing two
f32 methods it is an exact oracle to well beyond the precision that matters, and both methods are
measured against the same reference so any residual bias cancels in the ratio.
...
would only add to both methods. The 2.8x ratio is stable across three orders of magnitude of length,
which a noise artifact would not be.
